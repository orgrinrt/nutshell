...
# only move one way.
#
# Ten of twenty-eight today, eight without the shaking. That is a fact about
...
#
# It was six and eight when this was written. `deps` moved it to ten, because
# six closures were failing inside that one file.
#
...
it_ratchets_how_many_closures_reach_the_floor() {
    ...
    local v="ok"; [ "$shaken" -lt 10 ] && v="only ${shaken} of ${total} shaken"
    assert_eq "$v" "ok" "the shaken floor dropped below ten"

    v="ok"; [ "$whole" -lt 8 ] && v="only ${whole} of ${total} whole"
    assert_eq "$v" "ok" "the unshaken floor dropped below eight"
    ...
    # stops being in the file. Ten against eight today, and the direction is
